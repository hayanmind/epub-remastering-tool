...
5. **오픈소스 공개물**
   - GitHub 저장소 (핵심 모듈 전체 공개)
   - 기술 문서 6건 (README, ARCHITECTURE, API, FAQ, CONTRIBUTING, SIGIL-INTEGRATION)
...
| 빌드 | tsup (ESM/CJS, DTS 자동 생성) |
| 패키지 관리 | pnpm workspace (monorepo) |
| 인프라 | GCP (Cloud Run/GKE), GitHub Actions CI/CD |
...
|-------------|------|----------|------|
| Parser | `parser.test.ts` | 15개 | OPF/NCX/HTML 파서, ePub 전체 파싱 |
| Converter | `converter.test.ts` | 12개 | HTML5/CSS/OPF/Nav 변환, 패키징 |
| Accessibility | `accessibility.test.ts` | 10개 | KWCAG 2.1 태그 삽입, 메타데이터 |
| Validator | `validator.test.ts` | 11개 | ePubCheck, 접근성 점수, KPI 측정 |
| Pipeline | `pipeline.test.ts` | 12개 | 전체 파이프라인 통합 (Mock + Real) |
...
| 레벨 | 범위 | 도구 | 통과율 |
|------|------|------|--------|
| 단위 테스트 | 파서, 변환기, 접근성 모듈 | Vitest | 93% |
| 통합 테스트 | 파이프라인 모듈 간 연동 | Vitest | 90% |
| E2E 테스트 | 업로드->변환->다운로드 전체 흐름 | Vitest + Supertest | 100% |
| 접근성 테스트 | ePubCheck, Ace, KWCAG | CLI 도구 | 92% |
...
- 기술 문서 6건 완비 (README, ARCHITECTURE, API, FAQ, CONTRIBUTING, SIGIL-INTEGRATION)
...
| 4 | FAQ | docs/FAQ.md | 자주 묻는 질문 및 트러블슈팅 |
| 5 | 기여 가이드 | CONTRIBUTING.md | 개발 환경, 코딩 컨벤션, 테스트, 확장 포인트 |
| 6 | Sigil 연동 분석 | docs/SIGIL-INTEGRATION.md | Sigil 플러그인 시스템 분석 및 연동 전략 |
